...
Теги: Команда git switch поддерживает переключение только на ветки, но не на теги или отдельные коммиты.
...
Оба способа рабочие, но так как checkout напичкан большим количеством функций и флагом, его лучше не использовать.
"Команда git switch более выразительна и менее восприимчива к ошибкам. Кроме того, в ней меньше флагов и опций, поэтому ее легче запомнить."
...
но для удобства существует также команда pull, которая подтягивает и сливает изменения из удаленной ветки в текущую одним вызовом."
